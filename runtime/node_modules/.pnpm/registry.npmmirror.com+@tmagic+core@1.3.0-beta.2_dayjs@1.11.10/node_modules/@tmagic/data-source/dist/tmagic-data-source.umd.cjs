(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('events'), require('lodash-es'), require('@tmagic/utils')) :
  typeof define === 'function' && define.amd ? define(['exports', 'events', 'lodash-es', '@tmagic/utils'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.TMagicDataSource = {}, global.EventEmitter, global.lodashEs, global.utils));
})(this, (function (exports, EventEmitter, lodashEs, utils) { 'use strict';
    ...
    async request(options) {
      try {
        ...
        if (this.schema.responseOptions?.dataPath) {
          const data = utils.getValueByKeyPath(this.schema.responseOptions.dataPath, res);
          this.setData(data);
        } else {
          this.setData(res);
        }
  ...
  class DataSourceManager extends EventEmitter {
    ...
    compiledNode(node, sourceId) {
      ...
      if (node.visible === false)
        return node;
      return utils.compiledNode(
        (value) => {
          if (typeof value === "string") {
            return lodashEs.template(value)(this.data);
          }
    ...
    compliedConds(node) {
      ...
      for (const { cond } of node.displayConds) {
        ...
        for (const { op, value, range, field } of cond) {
          ...
          if (!utils.compiledCond(op, fieldValue, value, range)) {
            result = false;
            break;
          }
        }
  ...
  const createDataSourceManager = (app) => {
    ...
    if (dsl.dataSources && dsl.dataSourceCondDeps && platform !== "editor") {
      utils.getNodes(utils.getDepNodeIds(dsl.dataSourceCondDeps), dsl.items).forEach((node) => {
        node.condResult = dataSourceManager.compliedConds(node);
        utils.replaceChildNode(node, dsl.items);
      });
    }
    if (dsl.dataSources && dsl.dataSourceDeps) {
      utils.getNodes(utils.getDepNodeIds(dsl.dataSourceDeps), dsl.items).forEach((node) => {
        utils.replaceChildNode(dataSourceManager.compiledNode(node), dsl.items);
      });
    }
    dataSourceManager.on("change", (sourceId) => {
      const dep = dsl.dataSourceDeps?.[sourceId] || {};
      const condDep = dsl.dataSourceCondDeps?.[sourceId] || {};
      const nodeIds = lodashEs.union([...Object.keys(condDep), ...Object.keys(dep)]);
      dataSourceManager.emit(
        "update-data",
        utils.getNodes(nodeIds, dsl.items).map((node) => {
          const newNode = lodashEs.cloneDeep(node);
  ...
  exports.DataSourceManager = DataSourceManager$1;
...
//# sourceMappingURL=tmagic-data-source.umd.cjs.map
